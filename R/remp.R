...
remp <- function(methyDat, REtype = c("Alu", "L1"), parcel = NULL, 
  ...
  if ("naive" == method) {
    BEST_TUNE <- DataFrame(matrix(NA, nrow = sampleN, ncol = 0)) 
  ...
  for (i in seq_len(sampleN)) {
    ...
    if (method != "naive") {
      ...
      BEST_TUNE[i, ] <- as.numeric(P_basic$bestTune)
    } else {
      REMP_PREDICT_IMP <- DataFrame()
      BEST_TUNE <- DataFrame()
    }
    ...
    if (method != "naive" & BiocParallel::bpworkers(be) > 1) {
      ...
      REMP_PREDICT_CpG[, i] <- BiocParallel::bpiterate(ITER, .predictREMP, model = P_basic$model, BPPARAM = be, REDUCE = c)
      if (!is.null(P_basic$QC)) {
        ITER <- .iblkrow(newdata, chunks = ncore)
        REMP_PREDICT_QC[, i] <- BiocParallel::bpiterate(ITER, .predictQC, model = P_basic$QC, BPPARAM = be, REDUCE = c)
      }
      bpstop(be)
    } else {
      REMP_PREDICT_CpG[, i] <- .predictREMP(newdata, P_basic$model)
      if (!is.null(P_basic$QC)) 
        REMP_PREDICT_QC[, i] <- .predictQC(newdata, P_basic$QC) else REMP_PREDICT_QC <- NULL
    }
