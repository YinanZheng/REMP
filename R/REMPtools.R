...
remprofile <- function(methyDat, REtype = c("Alu", "L1"), RE = NULL, verbose = FALSE) {
  ...
  probeNames <-intersect(probeNames, names(ILMN.GR))
  
  methyDat <- methyDat[probeNames, , drop = FALSE]
  ILMN.GR <- ILMN.GR[probeNames]
  ...
  Profiled_RECpG_M <- methyDat[names(cpgRanges), , drop = FALSE]
  ...
  remproduct <- REMProduct(REtype = REtype, platform = arrayType, win = "N/A",
